...
async function startServer() {
  ...
  app.post("/api/posts", (req, res) => {
    try {
      const { id: bodyId, title, author, authorUid, authorEmail, category, tags, description, filename, sizeKb, passcode, content, imageUrl, likes, createdAt } = req.body;
      ...
      const posts = readDb();
      const id = (bodyId && String(bodyId).trim()) || ("p_" + Date.now().toString(36) + Math.random().toString(36).slice(2, 7));

      const newPost: Post = {
        id,
        title: String(title).substring(0, 80),
        author: String(author).substring(0, 40),
        ...
        filename: String(filename).substring(0, 100),
        sizeKb: Number(sizeKb) || 1,
        passcode: String(passcode).substring(0, 40),
  ...
  app.post("/api/posts/:id/like", (req, res) => {
    try {
      ...
      const uid = userId ? String(userId).trim() : "anonymous";
      posts[idx].likedBy = Array.isArray(posts[idx].likedBy) ? posts[idx].likedBy : [];
      if (!posts[idx].likedBy.includes(uid)) {
        posts[idx].likedBy.push(uid);
        posts[idx].likes = (posts[idx].likes || 0) + 1;
        writeDb(posts);
      }

      res.json({ id, likes: posts[idx].likes, likedBy: posts[idx].likedBy });
    } catch (err) {
  ...
  app.post("/api/posts/:id/unlike", (req, res) => {
    try {
      ...
      posts[idx].likedBy = Array.isArray(posts[idx].likedBy) ? posts[idx].likedBy : [];
      if (posts[idx].likedBy.includes(uid)) {
        posts[idx].likedBy = posts[idx].likedBy.filter((u: string) => u !== uid);
  ...
  app.post("/api/posts/:id/delete", (req, res) => {
    try {
      ...
      const isPasscodeMatch = Boolean(passcode && String(passcode).trim() === String(post.passcode).trim());

      // Authorization check: Admin OR valid Passcode match
      if (!isAdmin && !isPasscodeMatch) {
        return res.status(403).json({
          error: "Código de segurança incorreto. Digite o código cadastrado na criação do post."
        });
      }
  ...
  // POST: Send passcode recovery email
  app.post("/api/send-forgot-passcode", async (req, res) => {
    try {
      ...
      const targetEmail = (email || post.authorEmail || "").trim();
      if (!targetEmail) {
        return res.status(400).json({ error: "Nenhum e-mail foi cadastrado para esta construção." });
      }
      ...
      try {
        ...
        if (smtpHost) {
          ...
          await transporter.sendMail({
            from: process.env.SMTP_FROM || '"BlockFrame" <no-reply@blockframe.app>',
            to: targetEmail,
            subject: `[BlockFrame] Seu código de segurança para "${post.title}"`,
            text: `Olá!\n\nVocê solicitou o seu código de segurança para a construção "${post.title}" no BlockFrame.\n\n📌 Detalhes da Construção:\n- Título: ${post.title}\n- Nome do Arquivo: ${post.filename}\n- Categoria: ${post.category}\n\n🔑 Seu Código de Segurança: ${post.passcode}\n\nGuarde este código em local seguro para gerenciar ou excluir suas construções.\n\nAtenciosamente,\nEquipe BlockFrame`,
          });

          return res.json({
            success: true,
            message: `Código de segurança enviado com sucesso para ${targetEmail}! Verifique sua caixa de entrada e caixa de spam.`,
          });
        }
      } catch (err) {
      ...
      return res.json({
        success: true,
        message: `Código enviado para o e-mail cadastrado (${targetEmail}): Seu código de segurança é "${post.passcode}".`,
        code: post.passcode,
      });
    } catch (err) {
      res.status(500).json({ error: "Erro ao enviar código por e-mail" });
    }
  });
