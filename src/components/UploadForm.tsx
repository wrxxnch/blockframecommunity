...
import { Hammer, Tag, Folder, Sparkles, HelpCircle, FileText, Key, Check, Mail } from "lucide-react";
...
export default function UploadForm({ onSuccess }: UploadFormProps) {
  ...
  const processFile = async (selectedFile: File) => {
    if (!selectedFile?.name || !selectedFile.name.toLowerCase().endsWith(".bf")) {
      setStatus({
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      ...
      reader.onload = (event) => {
        const img = new Image();
        img.onload = () => {
          // Auto-resize to optimal thumbnail dimensions (max 640x480) with high quality
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newPost = await createPost({
        ...
        authorEmail: authorEmail.trim() || currentUser?.email || undefined,
        category,
        tags,
        description,
        filename: file.name,
      ...
      try {
        ...
        console.warn("Could not save created post IDs to localStorage:", err);
      }
    ...
    <div id="enviar" className="max-w-4xl mx-auto py-10 px-4">
      ...
      <form onSubmit={handleSubmit} className="mc-wood-panel p-6 md:p-8 rounded-sm shadow-2xl relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
          ...
          {/* Author Email (For password recovery) */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-pixel text-mc-gold tracking-wide flex items-center gap-1.5" htmlFor="authorEmail">
              <Mail className="w-3.5 h-3.5" />
              <span>Seu E-mail (para recuperar o código de segurança)</span>
            </label>
            <input
              id="authorEmail"
              type="email"
