...
import { Hammer, BarChart3, Users, Heart, Globe, LogIn, LogOut, User as UserIcon } from "lucide-react";
...
export default function MinecraftHeader({ posts }: MinecraftHeaderProps) {
  ...
  const uniqueAuthors = new Set(
    posts.map((p) => (p.author ? p.author.toLowerCase() : "anônimo")).filter(Boolean)
  ).size;
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-4 justify-center md:justify-end">
          ...
          <button
            ...
            className="mc-button flex items-center gap-2 text-xs font-pixel py-1.5 px-3 bg-neutral-800 border-neutral-600 hover:border-mc-gold transition cursor-pointer"
          ...
          {/* Google Login / Logout Section */}
          {currentUser ? (
            <div className="flex items-center gap-2 bg-neutral-800 border-2 border-neutral-700 px-3 py-1 rounded-sm">
              {currentUser.photoURL ? (
                <img
                  src={currentUser.photoURL}
                  alt={currentUser.displayName || "User"}
                  className="w-6 h-6 rounded-full border border-mc-gold"
                  referrerPolicy="no-referrer"
                />
              ) : (
                <UserIcon className="w-4 h-4 text-mc-gold" />
              )}
              <span className="text-xs font-pixel text-white max-w-[120px] truncate">
                {currentUser.displayName || currentUser.email?.split("@")[0]}
              </span>
              <button
                onClick={handleLogout}
                disabled={authLoading}
                className="mc-button text-[10px] py-1 px-2 bg-red-900 border-red-700 text-white hover:bg-red-800 flex items-center gap-1 cursor-pointer"
            ...
            <button
              onClick={handleLogin}
              disabled={authLoading}
              className="mc-button flex items-center gap-2 text-xs font-pixel py-1.5 px-3 bg-emerald-900 border-emerald-700 text-emerald-200 hover:bg-emerald-800 cursor-pointer transition"
              title="Entrar com conta Google"
            >
              <LogIn className="w-4 h-4 text-emerald-300" />
              <span>{authLoading ? "Entrando..." : "Entrar com Google"}</span>
            </button>
          )}
        </div>
      </div>
